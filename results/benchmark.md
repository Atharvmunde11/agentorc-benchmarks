# Agent ORC Benchmark Report

Generated **2026-07-14T11:25:28.934Z** · suite v1.0.0 · mode `mock` · scale `full` · adapter `undefined`
...
**What:** Startup Benchmark

**Why:** See methodology.

**How:** See detailed report.
...
```json
{
  "databasePath": "./data/insert-100.db",
...
```json
{
  "databasePath": "./data/insert-1000.db",
...
```json
{
  "databasePath": "./data/insert-10000.db",
...
```json
{
  "databasePath": "./data/insert-100000.db",
...
Notes:
- **Agent ORC:** Storage today is SQLite; PostgreSQL is planned.
- **Agent ORC:** Runs offline when you point embeddings/LLM at a local provider (e.g. Ollama).
- **Agent ORC:** Hybrid (keyword + vector) search is not in v1.
- **Chroma:** Single-node deployments use SQLite for metadata; vector segments are separate.
- **Chroma:** Hybrid / multi-retriever fusion appears in newer query planning docs; treat as Partial vs Agent ORC’s agent-memory focus.
- **Qdrant:** Typically runs as a server (Docker) or Qdrant Edge in-process — not a SQLite file SDK.
- **Qdrant:** Offers vector quantization; that is not the same as agent memory summarization.
- **Qdrant:** Documented dense + sparse hybrid search.
- **LanceDB:** Embedded OSS on the Lance columnar format.
- **LanceDB:** Documented hybrid vector + full-text search.
- **Mem0:** OSS supports local defaults (incl. SQLite history) and pluggable vector stores; hosted platform is also offered.
- **Mem0:** Markets memory extraction / condensation for token savings.
